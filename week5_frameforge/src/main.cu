...
// Host-side logger. Goes to stderr so it doesn't get mixed up with program output
// if you ever pipe stdout. Device printf goes to stdout — grep "[host]" / "[gpu]"
// to separate the two streams.
#define LOG(...)                                          \
  do {                                                    \
...
int main() {
    ...
    LOG("input ready — h_data[0..5] = %u %u %u %u %u %u",
        h_data[0], h_data[1], h_data[2], h_data[3], h_data[4], h_data[5]);
    ...
    LOG("gpu out      [0..5] = %.4f %.4f %.4f %.4f %.4f %.4f",
    ...
    LOG("cpu expected [0..5] = %.4f %.4f %.4f %.4f %.4f %.4f",
